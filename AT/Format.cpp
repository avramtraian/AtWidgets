...
{
    auto& offset = m_formatted.push_uninitialized(string.byte_count());
...
void FormatBuilder::push_integer(AT::u64 integer, IsNegative is_negative)
{
    if (integer == 0)
    {
        m_formatted.push_back('0');
        return;
    }

    if (is_negative == IsNegative::Yes)
        m_formatted.push_back('-');
    ...
    char* destination = &m_formatted.push_uninitialized(digit_count);
...
bool FormatBuilder::consume_until_format_specifier()
{
    ...
    const StringView string_to_copy =
        m_format.substring(copy_begin_offset, m_format_offset - copy_begin_offset);
...
{
    if (m_format_offset == 0 ||
        m_format.at_offset_in_bytes(m_format_offset - 1) != AT_FORMAT_SPECIFIER_BEGIN_TOKEN)
        return false;
    ...
    const StringView specifier_string =
        m_format.substring(begin_offset, m_format_offset - begin_offset);
    ++m_format_offset;
    return process_format_specifier(specifier_string, specifier);
}

bool FormatBuilder::process_format_specifier(
    AT::StringView specifier_string, AT::FormatBuilder::Specifier& specifier
)
{
